...
class Employee extends Model
{
    ...
    {
        // If you need direct access to salary generate
        return $this->hasManyThrough(
            SalaryGenerate::class,
            SalarySheet::class,
            'employee_id', // Foreign key on salary_sheets table
            'id', // Foreign key on salary_generates table
            'id', // Local key on employees table
            'salary_generate_id' // Local key on salary_sheets table
        );
    }
    ...
    public function termination()
    {
        return $this->hasOne(Termination::class, 'employee_id')
            ->where('status', 1); // Only active termination records
    }
}