...
/*!*********************************************!*\
  !*** ./resources/assets/js/ticket-reply.js ***!
  \*********************************************/
...
$(document).on('submit', '#ticketReplyStoreForm', function (e) {
  ...
  var reply = $('<div />').html($('#replyId').summernote('code'));
  var emptyReply = reply.text().trim().replace(/ \r\n\t/g, '') === '';

  if (emptyReply) {
    displayErrorMessage('Reply field is not contain only white space');
    return false;
  }
...
$(document).on('click', '.edit-ticket-reply', function () {
  var editTicketReplyID = $(this).attr('data-id');
  $.ajax({
...
$(document).on('submit', '#ticketReplyUpdateForm', function (e) {
  ...
  var reply = $('<div />').html($('#editReplyID').summernote('code'));
  var emptyReply = reply.text().trim().replace(/ \r\n\t/g, '') === '';
  ...
  var ticketReplyID = $('#ticketReplyID').val();
  $.ajax({
...
$(document).on('click', '.delete-ticket-reply', function () {
  var deleteTicketReplyID = $(this).attr('data-id');
  swal({
    title: Lang.get('messages.common.delete') + '!',
    text: Lang.get('messages.common.delete_confirm_common') + ' "' + 'Ticket Reply' + '"?',
    type: 'warning',
