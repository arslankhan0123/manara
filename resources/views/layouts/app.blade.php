...
<body>
    ...
    @include('layouts.lock_screen_modal')
    ...
    <script>
        ...
        let sessionTimeoutMinutes = parseInt("{{ \App\Models\Setting::where('key', 'session_timeout')->value('value') ?? 30 }}");
        let isScreenLocked = {{ session('is_locked') ? 'true' : 'false' }};
    </script>
    @yield('page_scripts')
    <script>
