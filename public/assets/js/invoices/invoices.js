...
/*!**************************************************!*\
  !*** ./resources/assets/js/invoices/invoices.js ***!
  \**************************************************/
...
$(document).ready(function () {
  ...
  setTimeout(function () {
    if (editData == true && $('.invoiceDueDate').val() !== '') {
      $('.invoiceDueDate').data('DateTimePicker').minDate($('.invoiceDueDate').val());
    } else {
  ...
  $(document).on('click', '#editSaveSend', function (e) {
    ...
    var formEdit = document.getElementById('editInvoiceForm');
  ...
  $(document).on('click', '#btnSaveAddress', function () {
    ...
    if (typeof createInvoiceAddress !== 'undefined' && createInvoiceAddress) {
      if (checkBillAddressFields().length == 0) {
        $('#bill_to').html('_ _ _ _ _ _');
      } else {
