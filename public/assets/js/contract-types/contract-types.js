...
/*!**************************************************************!*\
  !*** ./resources/assets/js/contract-types/contract-types.js ***!
  \**************************************************************/


var contractTypeUrl = route('contract-types.index') + '/';
var contractTypeCreateUrl = route('contract-types.store');
var tableName = '#contractTypesTable';
$(tableName).DataTable({
  oLanguage: {
  ...
  columns: [{
    ...
    data: function data(row) {
      ...
      return prepareTemplateRender('#contractTypeActionTemplate', data);
    },
    name: 'id'
  }]
});
...
$(document).on('click', '.edit-btn', function (event) {
  var contractTypeId = $(event.currentTarget).data('id');
  renderData(contractTypeId);
});

window.renderData = function (id) {
  $.ajax({
    url: route('contract-types.edit', id),
    type: 'GET',
    success: function success(result) {
      if (result.success) {
...
$(document).on('click', '.delete-btn', function (event) {
  var contractTypeId = $(event.currentTarget).data('id');
  deleteItem(contractTypeUrl + contractTypeId, '#contractTypesTable', Lang.get('messages.common.contract_type'));
});
