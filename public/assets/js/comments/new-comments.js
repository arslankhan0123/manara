...
/*!******************************************************!*\
  !*** ./resources/assets/js/comments/new-comments.js ***!
  \******************************************************/
...
function addCommentSection(comment) {
  ...
  if (comment.added_by == authId) {
    icons = '                    <a class="user__icons del-comment d-none task-comment-delete" title="' + Lang.get('messages.common.delete') + '" data-id="' + id + '"><i class="fas fa-trash ml-0 card-delete-icon"></i></a>\n' + '                    <a class="user__icons edit-comment d-none task-comment-edit" title="' + Lang.get('messages.common.edit') + '" data-id="' + id + '"><i class="fas fa-edit mr-2 card-edit-icon"></i></a>\n' + '                    <a class="user__icons save-comment comment-save-icon-' + id + ' d-none task-comment-check" data-id="' + id + '"><i class="far fa-check-circle card-save-icon text-success font-weight-bold hand-cursor mr-2 ml-2"></i></a>\n' + '                    <a class="user__icons cancel-comment comment-cancel-icon-' + id + ' d-none task-comment-cancel" data-id="' + id + '"><i class="fas fa-times card-cancel-icon hand-cursor"></i></a>\n';
  }

  return '<div class="activity clearfix comments__information" id="comment__' + id + '">\n' + '        <div class="activity-icon">\n' + '            <img class="user__img profile" src="' + comment.user.image_url + '" alt="User Image" width="50" height="50">\n' + '            <span class="user__username">\n' + '            </span>\n' + '        </div>\n' + '        <div class="activity-detail col-xl-11 col-lg-10 pt-1 mb-3">' + '        <div class="mb-0 d-flex justify-content-between flex-wrap">' + '        <div class="d-flex flex-wrap align-items-center">' + '             <span class="font-weight-bold lead">' + comment.user.full_name + '</span>\n' + '            <span class="user__description text-job text-dark ml-2">Just now</span>\n' + '        </div><div>' + icons + '            </div></div>' + '            <div class="user__comment mt-2 comment-display comment-display-' + id + '" data-id="' + id + '">\n' + comment.description + '           </div>\n' + '           <div class="user__comment d-none comment-edit comment-edit-' + id + '">\n' + '           <div id="commentEditContainer' + id + '" class="quill-editor-container"></div>\n' + '           </div>\n' + '       </div>' + '    </div>';
}

$(document).on('click', '#btnComment', function () {
  var description = quillComment.summernote('code');
  ...
  var commentDescription = $('<div />').html($('#commentContainer').summernote('code'));
  var empty = commentDescription.text().trim().replace(/ \r\n\t/g, '') === '';
...
function processAddCommentResponse(result) {
  var commentDiv = addCommentSection(result);
...
function processDeleteCommentResponse(commentId, ownerId) {
  var commentDiv = 'comment__' + commentId;
  $('#' + commentDiv).remove();
  $.ajax({
...
$(document).on('click', '.comment-display,.edit-comment', function () {
  ...
  if ($('#commentEditContainer' + commentId).html() === '') {
    ...
    var commentData = $.trim($('.comment-display-' + commentId).html());
...
window.setCommentEditData = function (commentId) {
  ...
  quillCommentEdit[commentId] = $('#commentEditContainer' + commentId).summernote({
    placeholder: Lang.get('messages.comment.add_comment') + '...',
    minHeight: 200,
...
$(document).on('click', '.save-comment', function (event) {
  var commentId = $(this).data('id');
  var description = quillCommentEdit[commentId].summernote('code'); // retrieve the HTML content from the Summernotecontainer

  var ownerId = $('#ownerId').val();
  var moduleId = $('#moduleId').val(); // this will check whether the SummernoteEditor has empty and doesn't have any content entered in it

  if (quillCommentEdit[commentId].summernote('isEmpty')) {
    return false;
  }

  var $editComment = $('<div />').html($('#commentEditContainer' + commentId).summernote('code'));
  var empty = $editComment.text().trim().replace(/ \r\n\t/g, '') === '';
...
var quillComment = $('#commentContainer').summernote({
  placeholder: Lang.get('messages.comment.add_comment') + '...',
  minHeight: 200,
  toolbar: [['style', ['bold', 'italic', 'underline', 'clear']], ['font', ['strikethrough']], ['para', ['paragraph']]]
});
