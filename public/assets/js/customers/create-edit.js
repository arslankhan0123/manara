...
/*!******************************************************!*\
  !*** ./resources/assets/js/customers/create-edit.js ***!
  \******************************************************/


var customerUrl = route('customers.index');
$(document).ready(function () {
  ...
  $('#currencyId,#countryId,#languageId,#billingCountryId,#shippingCountryId').select2({
    width: '100%'
  });
  ...
  if ($('#billingStreet').val() !== '' && $('#billingCity').val() !== '' && $('#billingState').val() !== '' && $('#billingZip').val() !== '') {
    $('#shippingAddressCheck').removeAttr('disabled');
  }

  $(document).on('keypress keyup', '#billingStreet, #billingCity, #billingState, #billingZip', function () {
    if ($('#billingStreet').val() === '' && $('#billingCity').val() === '' && $('#billingState').val() === '' && $('#billingZip').val() === '') {
  ...
  $(document).on('click', '#copyBillingAddress', function () {
    if ($('#shippingAddressCheck').prop('checked') === true) {
      ...
      $('#shippingCountryId').val($('#billingCountryId').val()).trigger('change.select2');
    } else {
      $('#shippingStreet').val('');
      $('#shippingCity').val('');
      $('#shippingState').val('');
      $('#shippingZip').val('');
      $('#shippingCountryId').val('').trigger('change.select2');
    }
  });
  $(document).on('change', '#customerId', function () {
    var urlLastString = window.location.href.substring(window.location.href.lastIndexOf('/') + 1);
    location.href = !isNaN(urlLastString) ? customerUrl + '/' + $(this).val() + '/profile' : customerUrl + '/' + $(this).val() + '/' + urlLastString;
  });
