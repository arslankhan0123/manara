...
class EstimateRepository extends BaseRepository
{
    ...
    public function prepareEstimateData($input)
    {
        ...
        if (isset($items['estimate_date']) && !empty($items['estimate_date'])) {
            try {
                $items['estimate_date'] = \Carbon\Carbon::createFromFormat('d-m-Y', $items['estimate_date'])->format('Y-m-d');
            } catch (\Exception $e) {
                // Fallback to Y-m-d if d-m-Y fails
                try {
                    $items['estimate_date'] = \Carbon\Carbon::parse($items['estimate_date'])->format('Y-m-d');
                } catch (\Exception $e) {
                    // Stay same if both fail
                }
            }
        }

        if (isset($items['estimate_expiry_date']) && !empty($items['estimate_expiry_date'])) {
            try {
                $items['estimate_expiry_date'] = \Carbon\Carbon::createFromFormat('d-m-Y', $items['estimate_expiry_date'])->format('Y-m-d');
            } catch (\Exception $e) {
                // Fallback to Y-m-d if d-m-Y fails
                try {
                    $items['estimate_expiry_date'] = \Carbon\Carbon::parse($items['estimate_expiry_date'])->format('Y-m-d');
                } catch (\Exception $e) {
                    // Stay same if both fail
                }
            }
