            ...
            function populateTable(attendanceData, customerId, projectId, month, year) {
                ...
                attendanceData.forEach((employee, index) => {
                    ...
                    if (employee.termination && employee.termination.date) {
                        terminationDate = new Date(employee.termination.date);
                        terminationDay = terminationDate.getDate();
                        terminationMonth = terminationDate.getMonth() + 1;
                        terminationYear = terminationDate.getFullYear();
                    }
                    ...
                    for (let day = 1; day <= daysInMonth; day++) {
                        ...
                        if (joinDate.setHours(0, 0, 0, 0) > date.setHours(0, 0, 0, 0)) {
                            hours = 0;
                        ...
                        // Set readonly for terminated, leave, or before join dates
                        var readonlyAttr = '';
            ...
            function updateTotals() {
                ...
                $('#manageAttendancesTable tbody tr').each(function() {
                    ...
                    $(this).find('td.day-input input').each(function() {
                        ...
                        // Skip Fridays in absent calculation (they're not working days)
                        if (dayName === 'Fri') {
                            // Friday hours count as overtime
                        ...
                        // Check termination
                        let isTerminated = $(this).data('terminated');
                        if (isTerminated === 1) {
                            totalAbsentHours += hoursPerDay;
                            return true;
                        }
                        ...
                        if (value > hoursPerDay) {
                            singleOvertime += (value - hoursPerDay);
                        }

                        // Add base hours (up to 8) to total
                        totalHours += Math.min(value, hoursPerDay);
                    });

                    // Add overtime to total hours
                    totalHours += singleOvertime;

                    // Get actual hours (always 208 for full month)
                    ...
                    $(this).find('.net_hours').val((totalHours - totalAbsentHours).toFixed(2));
                });
