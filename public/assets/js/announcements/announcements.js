...
/*!************************************************************!*\
  !*** ./resources/assets/js/announcements/announcements.js ***!
  \************************************************************/
...
var tableName = $('#announcementTable');
tableName.DataTable({
  oLanguage: {
  ...
  ajax: {
    ...
    data: function data(_data) {
      _data.status = $('#filterAnnouncementStatus').find('option:selected').val();
      _data.showToClient = $('#showToClientId').find('option:selected').val();
    }
  },
  columnDefs: [{
    'targets': [2],
  ...
  columns: [{
    ...
    data: function data(row) {
      if (row.date != null) {
        return moment(row.date, 'YYYY-MM-DD hh:mm:ss').format('Do MMM, Y HH:mm A');
      }
    },
    name: 'date'
  }, {
    data: function data(row) {
      ...
      return prepareTemplateRender('#announcementShowToClientTemplate', data);
    },
    ...
    data: function data(row) {
      ...
      return prepareTemplateRender('#announcementActionTemplate', data);
    },
    name: 'id'
  }],
...
$(document).on('submit', '#addNewForm', function (e) {
  ...
  var $description = $('<div />').html($('#createMessage').summernote('code'));
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        ...
        $('#editAnnouncementDate').val(moment(result.data.date).utc().format('YYYY-MM-DD HH:mm:ss'));
        $('#editMessage').summernote('code', result.data.message);
        if (result.data.show_to_clients === true) $('#editShowToClients').prop('checked', true);else $('#editShowToClients').prop('checked', false);
        if (result.data.status === true) $('#editStatus').prop('checked', true);else $('#editStatus').prop('checked', false);
...
$(document).on('submit', '#editForm', function (event) {
  ...
  var $editDescription = $('<div />').html($('#editMessage').summernote('code'));
...
$(document).on('click', '.delete-btn', function () {
  ...
  deleteItem(route('announcements.destroy', announcementId), '#announcementTable', Lang.get('messages.common.announcement'));
});
