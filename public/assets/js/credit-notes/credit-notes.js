...
/*!**********************************************************!*\
  !*** ./resources/assets/js/credit-notes/credit-notes.js ***!
  \**********************************************************/


var productUrl = route('products.index');
var creditNotesURL = route('credit-notes.store');
var creditNotesUrl = route('credit-notes.index') + '/';
var customerURL = route('get.creditnote.customer.address');
$(document).ready(function () {
  ...
  $(document).on('click', '#saveAsDraft, #saveAndSend', function (e) {
    ...
    submit = true;
    var saveForm = document.getElementById('creditNoteForm');
    ...
    $.ajax({
      ...
      success: function success(result) {
        if (result.success) {
          var _creditNoteId = result.data.id;
          window.location = creditNotesUrl + _creditNoteId;
        }
      },
  ...
  $(document).on('click', '#editSaveAsDraft, #editSaveAndSend', function (e) {
    ...
    e.preventDefault();
    var editForm = document.getElementById('editCreditNoteForm');
    ...
    formData.append('total_amount', totalAmount);
    formData.append('sub_total', $('#subTotal').text());
    var id = $('#hdnCreditNoteId').val();
    $.ajax({
      url: creditNotesUrl + id,
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
      ...
      success: function success(result) {
        if (result.success) {
          var _creditNoteId2 = result.data.id;
          window.location = creditNotesUrl + _creditNoteId2;
        }
      },
      error: function error(result) {
        displayErrorMessage(result.responseJSON.message);
  ...
  $(document).on('click', '#btnSave', function () {
    ...
    if (typeof createCreditNoteAddress !== 'undefined' && createCreditNoteAddress) {
      if (checkBillAddressFields().length == 0) {
        $('#bill_to').html('_ _ _ _ _ _');
      } else {
  ...
  $(document).on('click', '#markAsDraft, #markAsOpen, #markAsVoid, #markAsClosed', function () {
    var paymentStatus = $(this).data('status');
    $.ajax({
