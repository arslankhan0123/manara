...
/*!**************************************************!*\
  !*** ./resources/assets/js/tasks/create-edit.js ***!
  \**************************************************/
...
$(document).ready(function () {
  ...
  $(document).on('submit', '#createTasks, #editTasks', function () {
    ...
    var description = $('<div />').html($('#taskDescription').summernote('code'));
  ...
  setTimeout(function () {
    if (editTasks == true && $('#dueDate').val() !== '') {
      ...
      $('#dueDate').data('DateTimePicker').minDate(moment().millisecond(0).second(0).minute(0).hour(0));
    }
  }, 1000);
  ...
  $(document).on('change', '#relatedToId', function () {
    ...
    $('#ownerLabel').text($(this).children('option:selected').text() + ':');
    $('#relatedToForm').slideDown();
  });

  window.changeOwner = function (selector, id) {
    $.ajax({
      url: route('change-owner'),
      type: 'get',
      dataType: 'json',
      data: {
        id: id
      ...
      success: function success(data) {
        ...
        $.each(data.data, function (i, v) {
          ...
          element.innerHTML = v;
          $(selector).append($('<option></option>').attr('value', i).text(element.value));
        });
        ...
        if (typeof customerId != 'undefined' && typeof relatedTo != 'undefined' && customerId !== '' && relatedTo === 'Customer') {
