...
/*!************************************************!*\
  !*** ./resources/assets/js/notes/new-notes.js ***!
  \************************************************/
...
function addNoteSection(note) {
  ...
  if (note.added_by == authId) {
    icons = '                    <a class="user__icons del-note d-none task-comment-delete" title="' + Lang.get('messages.common.delete') + '" data-id="' + id + '"><i class="fas fa-trash ml-0 card-delete-icon"></i></a>\n' + '                    <a class="user__icons edit-note d-none task-comment-edit" title="' + Lang.get('messages.common.edit') + '" data-id="' + id + '"><i class="fas fa-edit mr-2 card-edit-icon"></i></a>\n' + '                    <a class="user__icons save-note comment-save-icon-' + id + ' d-none task-comment-check" data-id="' + id + '"><i class="far fa-check-circle card-save-icon text-success font-weight-bold hand-cursor mr-2 ml-2"></i></a>\n' + '                    <a class="user__icons cancel-note comment-cancel-icon-' + id + ' d-none task-comment-cancel" data-id="' + id + '"><i class="fas fa-times card-cancel-icon hand-cursor"></i></a>\n';
  }

  return '<div class="activity clearfix notes__information" id="note__' + id + '">\n' + '        <div class="activity-icon">\n' + '            <img class="user__img profile" src="' + note.user.image_url + '" alt="User Image" width="50" height="50">\n' + '            <span class="user__username">\n' + '            </span>\n' + '        </div>\n' + '        <div class="activity-detail col-xl-11 col-lg-10 pt-1 mb-3">' + '        <div class="mb-0 d-flex justify-content-between flex-wrap">' + '        <div class="d-flex flex-wrap align-items-center">' + '             <span class="font-weight-bold lead">' + note.user.full_name + '</span>\n' + '            <span class="user__description text-job text-dark ml-2">Just now</span>\n' + '        </div><div>' + icons + '            </div></div>' + '            <div class="user__comment mt-2 note-display comment-display-' + id + '" data-id="' + id + '">\n' + note.note + '           </div>\n' + '           <div class="user__comment d-none note-edit comment-edit-' + id + '">\n' + '           <div id="noteEditContainer' + id + '" class="quill-editor-container"></div>\n' + '           </div>\n' + '       </div>' + '    </div>';
}

$(document).on('click', '#btnNote', function () {
  var note = quillComment.summernote('code');
  ...
  var noteDescription = $('<div />').html($('#noteContainer').summernote('code'));
  var empty = noteDescription.text().trim().replace(/ \r\n\t/g, '') === '';
    ...
    $.ajax({
      ...
      success: function success(result) {
        ...
        swal({
          ...
          text: Lang.get('messages.common.note_delete'),
          type: 'success',
          confirmButtonColor: '#6777ef',
          timer: 2000
        });
...
function processAddNoteResponse(result) {
  var noteDiv = addNoteSection(result);
...
$(document).on('click', '.note-display,.edit-note', function () {
  ...
  if ($('#noteEditContainer' + noteId).html() === '') {
    ...
    var noteData = $.trim($('.comment-display-' + noteId).html());
...
window.setNoteEditData = function (noteId) {
  ...
  quillCommentEdit[noteId] = $('#noteEditContainer' + noteId).summernote({
    placeholder: Lang.get('messages.placeholder.add_note'),
    minHeight: 200,
...
$(document).on('click', '.save-note', function () {
  var noteId = $(this).data('id');
  var note = quillCommentEdit[noteId].summernote('code'); // retrieve the HTML content from the SummerNoteContainer
  ...
  var $editNote = $('<div />').html($('#noteEditContainer' + noteId).summernote('code'));
  var empty = $editNote.text().trim().replace(/ \r\n\t/g, '') === '';
...
var quillComment = $('#noteContainer').summernote({
  placeholder: Lang.get('messages.placeholder.add_note'),
  minHeight: 200,
  toolbar: [['style', ['bold', 'italic', 'underline', 'clear']], ['font', ['strikethrough']], ['para', ['paragraph']]]
});
