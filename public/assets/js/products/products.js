...
$(document).ready(function () {
  ...
  $('#taxSelectOne, #editTaxSelectOne').select2({
    width: '100%'
  });
  $('#taxSelectTwo,#editTaxSelectTwo').select2({
    width: '100%'
  });
...
$(tableName).DataTable({
  ...
  ajax: {
    ...
    data: function data(_data) {
      _data.group = $('#filter_group').find('option:selected').val();
    },
  ...
  columns: [{
    ...
    data: function data(row) {
      if (row.rate != 0) {
        return '<i class="' + currentCurrencyClass + '"></i>' + ' ' + getFormattedPrice(row.rate);
      }
...
$(document).on('submit', '#addNewForm', function (e) {
  ...
  var productDescription = $('<div />').html($('#productDescription').summernote('code'));
  var empty = productDescription.text().trim().replace(/ \r\n\t/g, '') === '';
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        ...
        $('#editProductDescription').summernote('code', result.data.description);
        $('#editRate').val(result.data.rate);
        $('.price-input').trigger('input');
        $('#editTaxSelectOne').val(result.data.tax_1_id).trigger('change');
        $('#editTaxSelectTwo').val(result.data.tax_2_id).trigger('change');
        $('#editProductGroup').val(result.data.item_group_id).trigger('change');
...
$(document).on('submit', '#editForm', function (event) {
  ...
  var editProductDescription = $('<div />').html($('#editProductDescription').summernote('code'));
  var empty = editProductDescription.text().trim().replace(/ \r\n\t/g, '') === '';
...
$(document).on('click', '.delete-btn', function (event) {
  var productId = $(event.currentTarget).data('id');
  deleteItem(route('products.destroy', productId), '#productsTable', Lang.get('messages.common.product_1'));
});
