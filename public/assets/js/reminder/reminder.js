...
/*!**************************************************!*\
  !*** ./resources/assets/js/reminder/reminder.js ***!
  \**************************************************/
  ...
  columns: [{
    data: function data(row) {
      if (row.owner_type == 'App\\Models\\Customer') {
        ...
        _element.innerHTML = row.user.full_name;
        return '<a href="' + route('contacts.index') + '/' + row.user.owner_id + '"><img src="' + row.user.image_url + '" class="thumbnail-rounded" data-toggle="tooltip" title="' + _element.value + '"></a>';
      }

      var element = document.createElement('textarea');
      element.innerHTML = row.user.full_name;
      return '<a href="' + route('members.index') + '/' + row.user.id + '"><img src="' + row.user.image_url + '" class="thumbnail-rounded" data-toggle="tooltip" title="' + element.value + '"></a>';
    },
    name: 'reminder_to'
  }, {
    data: function data(row) {
    ...
    data: function data(row) {
      ...
      return prepareTemplateRender('#reminderActionTemplate', data);
    },
    name: 'user.last_name'
  }],
...
window.checkReminderContent = function (elementSelector) {
  var elSelector = '#' + elementSelector;
...
$(document).on('submit', '#addNewForm', function (e) {
  e.preventDefault();
  if (!checkReminderContent('reminderDescription')) return;
  processingBtn('#addNewForm', '#btnCreateSave', 'loading');
  var createDescription = $('<div />').html($('#reminderDescription').summernote('code'));
  var empty = createDescription.text().trim().replace(/ \r\n\t/g, '') === '';
...
$(document).on('click', '.edit-reminder-btn', function (event) {
  var reminderId = $(event.currentTarget).data('id');
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        ...
        $('#editNotifiedDate').val(format(result.data.notified_date, 'YYYY-MM-DD HH:mm:ss'));
        $('#editReminderTo').val(result.data.reminder_to).trigger('change');
        $('#editReminderDescription').summernote('code', result.data.description);
...
$(document).on('submit', '#editForm', function (event) {
  ...
  if (!checkReminderContent('editReminderDescription')) return;
  processingBtn('#editForm', '#btnEditSave', 'loading');
  var id = $('#reminderId').val();
  var editDescription = $('<div />').html($('#editReminderDescription').summernote('code'));
...
$(document).on('click', '.delete-reminder-btn', function (event) {
  var reminderId = $(event.currentTarget).data('id');
  deleteItem(route('reminder.destroy', reminderId), '#reminderTbl', Lang.get('messages.ticket.reminder'));
});
