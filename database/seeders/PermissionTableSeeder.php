...
class PermissionTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $admin = User::find(1);
        if ($admin) {
            foreach (['Revokes', 'Safety Materials', 'Terms & Conditions'] as $type) {
                $permissionsToAssign = Permission::where('type', $type)->pluck('name')->toArray();
                $admin->givePermissionTo($permissionsToAssign);
            }
        }
    }
}
