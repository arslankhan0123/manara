...
/*!***************************************************************!*\
  !*** ./resources/assets/js/language_translate/create-edit.js ***!
  \***************************************************************/


var tableName = '#translatorManagerTbl';
$(tableName).DataTable({
  oLanguage: {
  ...
  columns: [{
    ...
    data: function data(row) {
      var editTranslatorUrl = route('language.translation', row.id);
      return "<a href=\"".concat(editTranslatorUrl, "\" class=\"text-decoration-none\">Edit Translation</a>");
    },
    name: 'id'
  }, {
    data: function data(row) {
      ...
      return prepareTemplateRender('#translatorManagerActionTemplate', data);
    },
    name: 'id'
  }]
});
...
$(document).on('click', '.delete-translate-language', function () {
  var languageId = $(this).attr('data-id');
  deleteItem(route('translation.manager.destroy', languageId), tableName, 'Language');
});
